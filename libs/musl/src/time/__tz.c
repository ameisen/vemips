...
static size_t scan_trans(long long t, int local, size_t *alt)
{
	...
	while (n > 1) {
		...
		if (local) off = (int32_t)zi_read32(types + 6 * idx[m-1]);
		if (t - off < (int64_t)x) {
			n /= 2;
		} else {
			a = m;
	...
	/* First and last entry are special. First means to use lowest-idx
	...
	if (a == 0) {
		...
		if (local) off = (int32_t)zi_read32(types + 6 * idx[a-1]);
		if (t - off < (int64_t)x) {
