...
#if defined(_LIBUNWIND_TARGET_MIPS32) || 1
/// Registers_ppc holds the register state of a thread in a 32-bit PowerPC
/// process.
class _LIBUNWIND_HIDDEN Registers_mips32 {
public:
  Registers_mips32();
  Registers_mips32(const void *registers);
  ...
  void        setFloatRegister(int num, double value);
  bool        validVectorRegister(int) const { return false; }
  v128        getVectorRegister(int num) const { return{}; }
  void        setVectorRegister(int num, v128 value) {};
  const char *getRegisterName(int num);
  void        jumpto();
  static int  lastDwarfRegNum() { return 112; }

  uint64_t  getSP() const         { return _registers.__gpregs[29]; }
  void      setSP(uint32_t value) { _registers.__gpregs[29] = value; }
  uint64_t  getIP() const         { return _registers.__pc; }
  void      setIP(uint32_t value) { _registers.__pc = value; }

private:
  struct mips32_thread_state_t {
    unsigned int __pc; // program counter
    unsigned int __gpregs[32]; // GPRs

    // TODO COP0 control registers
  };

  struct mips32_float_state_t {
    double __fpregs[32];

    //unsigned int __fpcsr;     /* floating point control register */
    //unsigned int pc; // program counter counter
  };
  ...
  mips32_thread_state_t _registers;
  mips32_float_state_t  _floatRegisters;
};

inline Registers_mips32::Registers_mips32(const void *registers) {
  static_assert((check_fit<Registers_mips32, unw_context_t>::does_fit),
                "mips32 registers do not fit into unw_context_t");
  memcpy(&_registers, static_cast<const uint8_t *>(registers),
         sizeof(_registers));
  static_assert(sizeof(mips32_thread_state_t) == 132,
                "expected float register offset to be 132");
  memcpy(&_floatRegisters,
         static_cast<const uint8_t *>(registers) + sizeof(mips32_thread_state_t),
         sizeof(_floatRegisters));
}

inline Registers_mips32::Registers_mips32() {
  memset(&_registers, 0, sizeof(_registers));
  memset(&_floatRegisters, 0, sizeof(_floatRegisters));
}

inline bool Registers_mips32::validRegister(int regNum) const {
  if (regNum == UNW_REG_IP)
    return true;
  if (regNum == UNW_REG_SP)
    return true;
  if (regNum < 0)
    return false;
  if (regNum <= UNW_MIPS32_R31)
    return true;
  return false;
}

inline uint32_t Registers_mips32::getRegister(int regNum) const {
  switch (regNum) {
  case UNW_REG_IP:
    return _registers.__pc;
  case UNW_REG_SP:
    return _registers.__gpregs[29];
  default:
    if (regNum >= UNW_MIPS32_R0 && regNum <= UNW_MIPS32_R31)
    {
      return _registers.__gpregs[regNum - UNW_MIPS32_R0];
    }
  }
  _LIBUNWIND_ABORT("unsupported mips32 register");
}

inline void Registers_mips32::setRegister(int regNum, uint32_t value) {
  switch (regNum) {
  case UNW_REG_IP:
    _registers.__pc = value;
    return;
  case UNW_REG_SP:
    _registers.__gpregs[29] = value;
    return;
  default:
    if (regNum >= UNW_MIPS32_R0 && regNum <= UNW_MIPS32_R31)
    {
      _registers.__gpregs[regNum - UNW_MIPS32_R0] = value;
      return;
    }
  }
  _LIBUNWIND_ABORT("unsupported mips32 register");
...
inline bool Registers_mips32::validFloatRegister(int regNum) const {
  return regNum >= UNW_MIPS32_F0 && regNum <= UNW_MIPS32_F31;
}

inline double Registers_mips32::getFloatRegister(int regNum) const {
  assert(validFloatRegister(regNum));
  return _floatRegisters.__fpregs[regNum - UNW_MIPS32_F0];
}

inline void Registers_mips32::setFloatRegister(int regNum, double value) {
  assert(validFloatRegister(regNum));
  _floatRegisters.__fpregs[regNum - UNW_MIPS32_F0] = value;
}

inline const char *Registers_mips32::getRegisterName(int regNum) {
  switch (regNum) {
  case UNW_REG_IP:
    return "pc";
  case UNW_REG_SP:
